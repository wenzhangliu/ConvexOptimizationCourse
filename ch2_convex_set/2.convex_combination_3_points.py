...
x_3 = np.array([3.0, 3.0])  # 点x_2的坐标（二维）
...
plt.ylim([0, 4.0])
plt.grid()
plt.plot(x_1, x_2, color='gray', linewidth=2)
plt.plot(x_1, x_3, color='gray', linewidth=2)
plt.plot(x_3, x_2, color='gray', linewidth=2)
...
plt.text(x_3[0]+0.2, x_3[1], r'$x_3$', fontsize=12)

# 初始凸组合点
initial_theta_1 = 0.0
initial_theta_2 = 0.0
initial_theta_3 = 1 - initial_theta_1 - initial_theta_2
initial_x = initial_theta_1 * x_1 + initial_theta_2 * x_2 + initial_theta_3 * x_3
...
ax_slider_2 = fig.add_axes([0.15, 0.05, 0.50, 0.03])
...
def update_1(val):
    ...
    theta_1 = slider_1.val
    theta_2 = slider_2.val  # 继承当前 slider2 的值
    slider_2.valmax = 1.0 - theta_1  # 动态调整 slider2 的最大值
...
def update_2(val):
    ...
    new_x = theta_1 * x_1 + theta_2 * x_2 + theta_3 * x_3  # 计算凸组合点坐标
    scatter_point.set_offsets(np.array([new_x]))  # 直接更新散点位置
    text_label.set_position((new_x[0] + 0.1, new_x[1]))  # 更新文本位置
    text_label.set_text(r"$\theta_1=$"+f"{round(theta_1, 2)}, " + r"$\theta_2=$"+f"{round(theta_2, 2)}, " + r"$\theta_3=$"+f"{round(theta_3, 2)}")  # 更新文本内容
